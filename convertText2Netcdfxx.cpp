...
// module load gcc mpich netcdf;  g++ convertText2Netcdf.cpp -o convertText2Netcdf -lnetcdf -g -lnetcdf_c++4
#define NC_ERR 2
...
void parseFileFieldData(std::stringstream& ss, std::string sFirst, 
  ...
  if(dataLine) {
    ...
    if(!(sFirst.empty() || sFirst == fieldName)) {
	    if((indP < numPtcls || !numPtcls) && indT == nthStep) {
	      data[nComp*indP+iComp] = std::stod(sFirst);
        if(debug)
        	printf("%s @ %d indP %d  iComp %d indT %d\n", 
        		sFirst.c_str(), nComp*indP+iComp,	indP, iComp, indT);
      }
      ++indT;
      if(debug)
        printf( "indT -> %d \n", indT);
    ...
    if(! ss.str().empty()) {
      while(ss >> sd) {
        ...
        if(expectEqual) {
	      ...
	      if((indP < numPtcls || !numPtcls) && indT == nthStep) {
	        data[nComp*indP+iComp] = std::stod(sd);
	        if(debug)
	        	printf("%s @ %d indP %d  iComp %d indT %d\n", 
	        		sd.c_str(), nComp*indP+iComp,	indP, iComp, indT);
	      }
        ++indT;
        if(debug)
        	printf( "indT -> %d\n", indT);
...
int main(int argc, char** argv) {
  ...
  float* pz = new float[np];
  float* pvx = new float[np];
  float* pvy = new float[np];
  float* pvz = new float[np];
  
  int sz = data.size();  
  printf("np %d %d \n", np, sz);
  for(int i=0; i<np ;++i) {
    px[i] = float(data[dof*i]);
    py[i] = float(data[dof*i + 1]);
    pz[i] = float(data[dof*i + 2]);
    pvx[i] = float(data[dof*i + 3]);
    pvy[i] = float(data[dof*i + 4]);
    pvz[i] = float(data[dof*i + 5]);
  }
  ...
  NcVar rt = ncFile.addVar("rate", ncFloat, cons);
  NcVar xx = ncFile.addVar("x", ncFloat, pnp);
  NcVar yy = ncFile.addVar("y", ncFloat, pnp);
  NcVar zz = ncFile.addVar("z", ncFloat, pnp);
  NcVar vxx = ncFile.addVar("vx", ncFloat, pnp);
  NcVar vyy = ncFile.addVar("vy", ncFloat, pnp);
  NcVar vzz = ncFile.addVar("vz", ncFloat, pnp);
  xx.putVar(&px[0]);
  yy.putVar(&py[0]);
  zz.putVar(&pz[0]);
  vxx.putVar(&pvx[0]);
