...
int main(int argc, char** argv) {
  ...
  if(np>0)
   printf("@1 pos: %g %g %g v: %g %g %g \n", px[1], py[1], pz[1], pvx[1], pvy[1], pvz[1]);
  ...
  */
  int ret, ncid, dimid, constid, varconst, varidx, varidy, varidz;
  ...
  if ((ret = nc_def_var(ncid, "rate", NC_FLOAT, 0, &constid, &varconst)))
    ERR(ret);
